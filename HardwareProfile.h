 ...
 * 06/20/12     Modified for micromedia board	AB
 ********************************************************************/
 ...
 *
 *  When none of the two are defined prior to this check, the default
...
// error checks
    ...
    #endif
#elif defined (MIKRO_BOARD) || defined (mikromedia_BOARD)
    #define USE_SELF_POWER_SENSE_IO   RB10
    #define POWER_ADC_INPUT_SEL       AD1CHSbits.CH0SA
    #define ADPCFG_POWER              AD1PCFGbits.PCFG10

    #if defined(__PIC24F__)   // Battery charge voltage Sense RB8
        #define tris_self_power     TRISBbits.TRISB8    // VCC-BAT / 3
    #else
        #define tris_self_power     TRISBbits.TRISB10   // Input, Analog?
    #endif
...
*/
    ...
    !defined (MIKRO_BOARD)     &&           \
    !defined (mikromedia_BOARD)
        #error "Error: Graphics Hardware Platform is not defined! One of the following boards (GFX_PICTAIL_V3, GFX_PICTAIL_V3E, PIC24FJ256DA210_DEV_BOARD, MEB_BOARD, GFX_PICTAIL_LCC, MIKRO_MMB, mikromedia_BOARD) must be defined."
#endif
...
/*
    ...
    * MIKRO_MMB - MultiMedia Board from MikroElectronika for PIC24 or PIC32
    * mikromedia_BOARD - mikromedia Board from MikroElectronika for PIC24 or PIC32
*/
#if defined(PIC24FJ256DA210_DEV_BOARD)
...
#if defined(mikromedia_BOARD) || defined(MIKRO_BOARD)
    #define      USE_M25P80                     // use the 8 Mbit SPI Serial Flash
// fix this    #define      USE_TOUCHSCREEN_RESISTIVE      // use 4-wire resistive touch screen driver
    #define      USE_ACCELEROMETER_ADXL345      // use the accelerometer module
#endif
...
// Using HX8347D Display Controller
    ...
    #define DISP_HOR_RESOLUTION 240
    #define DISP_VER_RESOLUTION 320
#endif // #if (DISPLAY_CONTROLLER == HX8347D)

// Using SSD1963 Display Controller
#if defined(GFX_USE_DISPLAY_CONTROLLER_SSD1963)
    #define DISP_ORIENTATION    0
    #define DISP_HOR_RESOLUTION 480
    #define DISP_VER_RESOLUTION 272
#endif // #if (DISPLAY_CONTROLLER == SSD1963)
...
*********************************************************************/
    ...
    // optimum (fastest) timing requirement for your interface.
        ...
        #define PMP_DATA_SETUP_TIME     (15)    // See HX8347 data sheet for details.
        #define PMP_DATA_WAIT_TIME      (24)    //(45)  // Write cycle time 66 ns,
        #define PMP_DATA_HOLD_TIME      (21)    // Read access time 40 ns from register, 340 ns from Frame memory.
                                                // Read strobe H duration 90 ns, L duration 45 ns, 355 ns
        #define PMP_READ_SETUP_TIME     (45)    //(15)  // HX8347D has much longer access time in Read !
        #define PMP_READ_WAIT_TIME      (355)   // Read strobe L duration 45 ns, 355 ns, Read access time 40 ns, 340 ns
        #define PMP_READ_HOLD_TIME   	(74)    //(90)  // Read strobe H duration 90 ns,
                                                // Read cycle time 160 ns from register, 450 ns from Frame memory
    #elif defined (GFX_USE_DISPLAY_CONTROLLER_SSD1963)
        #define PMP_DATA_SETUP_TIME     (18)    //(50)  // See SSD1963 data sheet for details.
        #define PMP_DATA_WAIT_TIME  	(82)    //(45)  // Write cycle time  ns,
        #define PMP_DATA_HOLD_TIME      (0)     //(30)  // Read access time  ns from register,  ns from Frame memory.
                                                // Read cycle time  ns from register,  ns from Frame memory
    #else
        #error "USE_GFX_PMP is defined but no timing values are defined for the selected hardware interface. Define the following timing parameters: PMP_DATA_SETUP_TIME, PMP_DATA_WAIT_TIME, PMP_DATA_HOLD_TIME."
...
// Depending on the display panel used, backlight control may be different.
// This portion sets up the correct backlight level to turn backlight on/off.
#if defined (GFX_USE_DISPLAY_PANEL_TFT_G240320LTSW_118W_E)
...
/*
    ...
    - MikroElektronika MultiMediaBoard (MikroE MMB PIC32MX4)
 */

#elif defined (mikromedia_BOARD) || defined(MIKRO_MMB_PIC32MX4) || defined (MIKRO_BOARD)
    #if defined(__PIC24FJ256GB110__)

        // Definitions for reset pin LCD-RST	RC1
        #define DisplayResetConfig()        TRISCbits.TRISC1 = 0
        #define DisplayResetEnable()        LATCbits.LATC1 = 0
        #define DisplayResetDisable()       LATCbits.LATC1 = 1

        // Definitions for RS pin	LCD-RS	RB15
        ...
        // Definitions for CS pin	LCD-CS#	RF12
        ...
        #if defined (mikromedia_BOARD)
            // Definitions for backlight control LCD-BLED pin RD2
            #define DisplayBacklightConfig()	TRISDbits.TRISD2 = 0
            #define DisplayBacklightOn()    	LATDbits.LATD2 = 1
            #define DisplayBacklightOff()   	LATDbits.LATD2 = 0
        #else
            // Definitions for backlight control pin
            #define DisplayBacklightConfig()
            #define DisplayBacklightOn()
        ...
        // Display DNC aka RS pin connected to PMP Address line 0
        #define DeviceAddrConfig()  		PMAEN  = 1
        #define DeviceSetIndex()    		PMADDR = 0
        #define DeviceSetValue()    		PMADDR = 1

    #elif defined(__PIC32MX__)
        // Disable all display lines
        #define DisplaySetALLInputs()       TRISGSET = 0x0003; \
                                            TRISFSET = 0x1003; \
                                            TRISESET = 0x00FF; \
                                            TRISDSET = 0x30F0; \
                                            TRISCSET = 0x0002; \
        ...
        // Definitions for reset pin LCD-RST# (pin RC1)
        #define DisplayResetConfig()        TRISCCLR = _TRISC_TRISC1_MASK
        #define DisplayResetEnable()        LATCCLR = _LATC_LATC1_MASK
        #define DisplayResetDisable()       LATCSET = _LATC_LATC1_MASK

        // Definitions for LCD-RS (TFT-D/C#) (pin RB15/AN15/PMA0)
        ...
        // Definitions for LCD-CS# (pin RF12)
        #define DisplayConfig()             TRISFCLR = _TRISF_TRISF12_MASK
        #if defined (__DEBUG)
	    #define DisplayEnable()         LATFCLR = _LATF_LATF12_MASK; LATACLR = 2
	    #define DisplayDisable()        LATFSET = _LATF_LATF12_MASK; LATASET = 2
	#else
	    #define DisplayEnable()         LATFCLR = _LATF_LATF12_MASK
	    #define DisplayDisable()        LATFSET = _LATF_LATF12_MASK
	#endif

        // Definitions for LCD-WR# (write) (pin RD4)
        #define WRConfig()           TRISDCLR = _TRISD_TRISD4_MASK
        #define WRDisable()          LATDSET = _LATD_LATD4_MASK
        #define WREnable()           LATDCLR = _LATD_LATD4_MASK

        // Definitions for LCD-RD# (read) (pin RD5)
        #define RDConfig()           TRISDCLR = _TRISD_TRISD5_MASK
        #define RDDisable()          LATDSET = _LATD_LATD5_MASK
        #define RDEnable()           LATDCLR = _LATD_LATD5_MASK

        // Definitions for DATA pins RG[1:0],RF[1:0],RE[7:0],RD[13,12,7,6]
        #define DisplayDATAEnable()         TRISGCLR = 0x0003; \
                                            TRISFCLR = 0x0003; \
                                            TRISECLR = 0x00FF; \
                                            TRISDCLR = 0x30C0

        // Definitions for POWER ON pin RB10 VSENSE
        #define DisplayPowerConfig()
        #define DisplayPowerOn()
        #define DisplayPowerOff()

        #if defined (mikromedia_BOARD)
            // Definitions for FLASH CS# pin RA9
            #define DisplayFlashConfig()
            #define DisplayFlashEnable()
            #define DisplayFlashDisable()

            // Definitions for backlight control LCD-BLED pin RA9
            // This is taken care of by the SSD1963 driver...
//            #define DisplayBacklightConfig()    TRISACLR = _TRISA_TRISA9_MASK
//            #define DisplayBacklightOn()        LATASET = _LATA_LATA9_MASK
//            #define DisplayBacklightOff()       LATACLR = _LATA_LATA9_MASK
        #endif

        #if defined MIKRO_MMB_PIC32MX4
            // Definitions for FLASH CS pin EE-CS#
            #define DisplayFlashConfig()        TRISCbits.TRISC2 = 0
            #define DisplayFlashEnable()        LATCbits.LATC2 = 0
            #define DisplayFlashDisable()       LATCbits.LATC2 = 1

            // Definitions for backlight control LCD_BLED pin RD12
            #define DisplayBacklightConfig()    TRISDbits.TRISD12 = 0
            #define DisplayBacklightOn()        LATDbits.LATD12 = 1
            #define DisplayBacklightOff()       LATDbits.LATD12 = 0
        #endif

        // Display DNC aka RS pin controlled as PMP Address line 0
        #define DeviceAddrConfig()      PMAEN  = 1
        #if defined (__DEBUG)
            #define DeviceSetIndex()    PMADDR = 0; LATACLR = 1
            #define DeviceSetValue()    PMADDR = 1; LATASET = 1
        #else
            #define DeviceSetIndex()    PMADDR = 0
            #define DeviceSetValue()    PMADDR = 1
        #endif
    ...
    #endif	// (__PIC32MX__)
#endif	// end of #elif defined (mikromedia_BOARD) || defined(MIKRO_MMB_PIC32MX4)
...
*/
    ...
    #elif defined(__PIC32MX__)  // PIC32MX has selection field in Upper part of register.
        #define TOUCH_ADC_INPUT_SEL  AD1CHSbits.CH0SA
        #define TOUCH_ADC_DIFF_SEL   AD1CHSbits.CH0NA	// Differential reference setting

    #else // all other supported devices
        #define TOUCH_ADC_INPUT_SEL  AD1CHS
        #define TOUCH_ADC_DIFF_SEL   AD1CHSbits.CH0NA	// Differential reference selection
    #endif
...
// fix this: ugghhh... formatting
	...
	#elif defined(mikromedia_BOARD) || defined(MIKRO_BOARD)
	    #if defined(__PIC24FJ256GB110__) || defined(__PIC32MX__)
            #define ADC_XPOS    13
            #define ADC_YPOS    12

		//	#define ADC_VREF        180             // External voltage reference 1.8 V on mikromedia board.
		//	#define ADC_PR0		0               // External Pressure sensor on AN0, RB0
		//	#define ADC_PR0_PCFG AD1PCFGbits.PCFG0

		//	#define ADC_PR0_DIFF    1               // Differential between AN0 and AN1, RB1
		//	#define ADC_PR1		1               // External Pressure sensor on AN1, RB1
		//	#define ADC_PR1_PCFG AD1PCFGbits.PCFG1

			#define ADC_TMP		9               // External Thermometer LM35DZ on AN2, RB9
			#define ADC_TMP_PCFG AD1PCFGbits.PCFG9
								// Battery charge voltage Sense

		//	#define ADC_POT		5		// Potentiometer connected to external Analog port
		//	#define ADC_POT_PCFG AD1PCFGbits.PCFG5
		  #if defined(__PIC24F__)
			// Battery charge voltage Sense RB8
			#define ADC_BAT		 8		// VCC-BAT / 3
			#define ADC_BAT_PCFG AD1PCFGbits.PCFG8

		//	#define ADC_VBG		17		// Band gap voltage calibration.
		//	#define ADC_VBG_PCFG AD1PCFGHbits.PCFG17

		  #elif defined(__PIC32MX__)
			// Battery charge voltage Sense RB10
			#define ADC_BAT		 10		// VCC-BAT / 3
			#define ADC_BAT_PCFG AD1PCFGbits.PCFG10
		  #endif
/*
	    // X port definitions. Keep for Compatibility!
            ...
            // Y port definitions. Keep for Compatibility!
...
*/
            #endif
	#endif // end of #elif defined(mikromedia_BOARD) || defined(MIKRO_BOARD)
	...
	*********************************************************************/
	...
	/* ----------------------------------------- */
		...
		#endif
	#elif defined (MIKRO_BOARD) || defined (mikromedia_BOARD)
        #define TOUCHSCREEN_RESISTIVE_PRESS_THRESHOLD           924
 ...
 * SPI Flash Memory on mikromedia_BOARD
 *********************************************************************/
// Set up the signals used to communicate to the SPI Flash device
// define the SPI channel to be used
    #if defined (mikromedia_BOARD) || defined (MIKRO_BOARD)
    ...
    */
	...
	#if defined (__PIC32MX__)
            #define SST25_SPI_CHANNEL 1
            #define SST25_CS_LAT    LATAbits.LATA9
            #define SST25_CS_TRIS   TRISAbits.TRISA9
            #define SST25_SCK_TRIS  TRISDbits.TRISD10   // SCK SCK2-RD10
            #define SST25_SDI_TRIS  TRISCbits.TRISC4    // SDO MISO2-RC4
            #define SST25_SDO_TRIS  TRISDbits.TRISD0    // SDI MOSI2-RD0
	#endif
    #endif
...
*	Micro board has no user interface Buttons, but may well connect buttons to external ports.
*********************************************************************/
typedef enum
{
    HW_BUTTON_PRESS = 0,
...
#elif defined(mikromedia_BOARD) || defined (MIKRO_BOARD)
    #define TX1_TRIS TRISFbits.TRISF5
    #define RX1_TRIS TRISFbits.TRISF4
    #define TX2_TRIS TRISFbits.TRISF8
    #define RX2_TRIS TRISFbits.TRISF2
#else
...
*********************************************************************/
#define RTCC_DEFAULT_DAY        30      // 30th
#define RTCC_DEFAULT_MONTH      7       // July
#define RTCC_DEFAULT_YEAR       85      // 1985
